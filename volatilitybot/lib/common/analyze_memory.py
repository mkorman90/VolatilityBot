...
from volatilitybot.conf.config import CODE_EXTRACTORS
...
def analyze_memory(machine_instance, malware_sample):
    ...
    logging.info(
        '[{}] executing code extractors... (Sample ID: {})'.format(machine_instance.machine_name, malware_sample.sample_data['sha256']))
    ...
    logging.info(
        '[{}] Processing of memory done (Sample ID: {})'.format(machine_instance.machine_name, malware_sample.sample_data['sha256']))

    result = {'sample_data': malware_sample.sample_data}
    return result
