...
from conf.config import DEFAULT_SLEEP_TIME
from lib.common.analyze_memory import analyze_memory
from lib.common.utils import agent_send_sample
...
class Machine:
    ...
    def send_malware_sample(self, malware_sample):
        ...
        :return:
        """
        logging.info('Going to send malware sample {} to machine {} with ip {}'.format(malware_sample.id,self.machine_name,self.ip_address))
    ...
    def handle_malware_sample(self, malware_sample):
        ...
        :return:
        """
        logging.info('Machine {}  Will handle sample ID {}'.format(self.machine_name, malware_sample.id))

        self.status = 'executing'
        result = None

        logging.info('[{}] Reverting (Sample ID: {})'.format(self.machine_name, malware_sample.id))
        if self.revert() and self.start():
            ...
            logging.info(
                'Machine {} finished processing sample ID {} result: {}'.format(self.machine_name, malware_sample.id,
                                                                                result))
        status = 'failed'

        if result is not None:
            with open(os.path.join(os.path.dirname(os.path.realpath(malware_sample.file_path)),
                                   'report' + str(malware_sample.id) + '.json'), 'w+') as result_file:
    ...
    def show_info(self):
        print(
            'Machine Name: {}\n\tis_64bit: {}\n\tActive: {}\n\tSnapshot Name: {}\n\tStatus: {}\n\tMemory Profile: {}\n\tIP: {}'.format(
                self.machine_name, self.is_64bit, self.active, self.snapshot_name, self.status, self.memory_profile, self.ip_address))
